...
const Page = () => {
    ...
    const getSearchData = async (e) => {
        e.preventDefault()
        try {
            const fetchItems = await fetch(`http://localhost:5000/api/admin/order/searchorder/${searchvalue}`, {
                method: 'GET',
                headers: {
    ...
    const fetchAllOrders = async () => {
        try {
            const fetchItems = await fetch('http://localhost:5000/api/admin/order/discardedorders', {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'token': sessionStorage.getItem('token')
