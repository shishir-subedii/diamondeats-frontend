...
const Page = ({ params }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/food/singlefood/${id}`, {
        method: 'GET',
        headers: {
      ...
      if (data.success) {
        ...
        setSrc(`http://localhost:5000/public/images/${data.message.Picture}`)
      }
      else {
        alert(data.message)
        router.push('/menu')
    ...
    <main className={styles.viewFoodBox}>
      <section className={styles.foodItem}>
        ...
        <div className={styles.foodItem__details}>
          ...
          <button onClick={() => handleSum('minus')}>-</button> <button onClick={() => handleSum('plus')}>+</button> <br />
          {
